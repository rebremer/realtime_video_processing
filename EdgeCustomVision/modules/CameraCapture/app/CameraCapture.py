...
class CameraCapture(object):
                ...
                try:
                    if self.nbOfPreprocessingSteps == 0:
                        if self.verbose and (perfForOneFrameInMs is not None):
                            cv2.putText(frame, "Graffiti: " + str(round(json.loads(response)[0]["Probability"],2)),(10, 35),cv2.FONT_HERSHEY_SIMPLEX,1.0,(0,0,255), 2) #str(round(1000/perfForOneFrameInMs, 2)),(10, 35),cv2.FONT_HERSHEY_SIMPLEX,1.0,(0,0,255), 2)
                            if round(json.loads(response)[0]["Probability"],2) > 0.8 and uploaded == 0:
                                self.uploadVideo()
                                uploaded=1
                        if self.annotate:
                        ...
                        if self.verbose and (perfForOneFrameInMs is not None):
                            cv2.putText(preprocessedFrame, "Graffiti: " + str(round(json.loads(response)[0]["Probability"],2)),(10, 35),cv2.FONT_HERSHEY_SIMPLEX,1.0,(0,0,255), 2) # "FPS " + str(round(1000/perfForOneFrameInMs, 2)),(10, 35),cv2.FONT_HERSHEY_SIMPLEX,1.0,(0,0,255), 2)
                            if round(json.loads(response)[0]["Probability"],2) > 0.8 and uploaded == 0:
                                self.uploadVideo()
                                uploaded=1
                        if self.annotate:
    ...
    def __exit__(self, exception_type, exception_value, traceback):
        ...
        if self.showVideo:
            print ("do not close imageserver")
