# Copyright (c) Microsoft. All rights reserved. fr122
...
def main(
    ...
    '''
    Capture a camera feed, send it to processing and forward outputs to EdgeHub test2
    ...
    try:
        print ( "\nPython %s\n" % sys.version )
        print ( "Camera Capture Azure IoT Edge Module. Press Ctrl-C to exitsdf.")
        try:
            global hubManager
...
if __name__ == '__main__':
    try:
        if os.environ['VIDEO_PATH'] == "/dev/video0/video.json":
           with open(os.environ['VIDEO_PATH']) as json_file:  
